...
const AdminLogin = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // using axios to fetch the admin api
      ...
      const auth = response.data; // (data) props all the children inside
      console.log(auth);

      if (auth.token) {
        localStorage.setItem("token", auth.token); // grab the token
        setTimeout(() => {
          localStorage.removeItem("token");
          alert("Token has expired!, Please login again.");
          navigate("/");
        }, 3600 * 1000); // after an hour this token will expire
      }
